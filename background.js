...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.type === 'REFINE_PROMPT') {
    ...
    if (!ANTHROPIC_API_KEY || ANTHROPIC_API_KEY === 'YOUR_ANTHROPIC_API_KEY_HERE') {
      console.warn('[Fromptly Background] API Key not set - using fallback');

      // Fallback: 하드코딩 제안
      sendResponse({
        suggestions: {
          suggestionA: `${userPrompt}에 대해 더 구체적인 CSS 속성과 애니메이션 시간을 명시해주세요`,
          suggestionB: `${userPrompt}를 다른 방식으로 구현하려면 Flexbox 또는 Grid 레이아웃을 사용해보세요`
        }
      });
    ...
    // Claude API 호출 (비동기)
    refinePromptWithClaude(userPrompt)
      .then(suggestions => {
